...
const HoldingsTable: React.FC<HoldingsTableProps> = ({}) => {
  ...
  const { holdings, isLoading } = useGetUserPortfolioQuery(undefined, {
    selectFromResult: ({ data, error, isLoading, isFetching }) => {
      if (data) {
        if (selectedPortfolioId) {
          const selectedPortfolio =
            portfolioSelectors.selectById(data.portfolios, selectedPortfolioId) ||
            portfolioItemSelectors.selectById(data.portfolioItems, selectedPortfolioId)
          if (selectedPortfolio) {
            if ('parentId' in selectedPortfolio) {
              return {
                holdings: portfolioHoldingSelectors
                  .selectAll(data.holdings)
                  .filter(h => h.parentId === selectedPortfolio.id),
                error,
                isLoading,
                isFetching
              }
              ...
              return {
                holdings: portfolioHoldingSelectors
                  .selectAll(data.holdings)
                  .filter(h => selectedPortfolio.itemIds.includes(h.parentId)),
                error,
                isLoading,
                isFetching
              }
