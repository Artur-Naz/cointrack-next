    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials, req) {
                try {
                    const { data } = await axios.post('http://154.53.56.67:8008/api/v1/auth/login',{
...
async function refreshAccessToken(token: any) {
    try {
        const { data:refreshedTokens } = await axios.post('http://154.53.56.67:8008/api/v1/auth/refresh',{
...
async function getProfile(accessToken?: string){
    ...
    try {
        const { data:user } = await axios.get('http://154.53.56.67:8008/api/v1/auth/profile', {
            headers: {
